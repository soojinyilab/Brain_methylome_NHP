...
out_file_dir = "../summary_stat_all_ortho_cytosine/3_species_methyl_table/"
stypes = ["CG","CH"]
for stype in stypes:
    sfile = in_file_dir+"HCM_"+stype+"_avgMeth.txt"
    with open("tmp_"+stype+"_ortho_cytosine_process.sh",'w') as fout:
        systemstr = "tail -n +2 "+sfile+" | awk -v OFS='\\t' '"+'{print $1,$2-1,$2,$3}'+"' > "+out_file_dir+"Human_ND_"+stype+"_avgMeth.bed\n"
        systemstr = systemstr + "tail -n +2 "+sfile+" | awk -v OFS='\\t' '"+'{print $1,$2-1,$2,$4}'+"' > "+out_file_dir+"Chimp_ND_"+stype+"_avgMeth.bed\n"
        systemstr = systemstr + "tail -n +2 "+sfile+" | awk -v OFS='\\t' '"+'{print $1,$2-1,$2,$5}'+"' > "+out_file_dir+"Macaq_ND_"+stype+"_avgMeth.bed\n"
        systemstr = systemstr + "tail -n +2 "+sfile+" | awk -v OFS='\\t' '"+'{print $1,$2-1,$2,$6}'+"' > "+out_file_dir+"Human_OD_"+stype+"_avgMeth.bed\n"
        systemstr = systemstr + "tail -n +2 "+sfile+" | awk -v OFS='\\t' '"+'{print $1,$2-1,$2,$7}'+"' > "+out_file_dir+"Chimp_OD_"+stype+"_avgMeth.bed\n"
        systemstr = systemstr + "tail -n +2 "+sfile+" | awk -v OFS='\\t' '"+'{print $1,$2-1,$2,$8}'+"' > "+out_file_dir+"Macaq_OD_"+stype+"_avgMeth.bed\n"
        fout.write(systemstr)
        
    run_systemstr = 'echo "cd '+wd+' && source activate stats && '
    run_systemstr = run_systemstr + "bash "+"tmp_"+stype+"_ortho_cytosine_process.sh && "
    run_systemstr = run_systemstr + "rm "+"tmp_"+stype+"_ortho_cytosine_process.sh"
