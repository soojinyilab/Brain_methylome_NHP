...
#DMLtest = DMLtest.multiFactor(DMLfit, term="Age_Class:Species")
...
write.table(DMLtest[ix,],file=paste0(args[2],"_DML_Age_fdrs.txt"),sep="\t",row.names=F,quote=F)
...
write.table(DMRtest,file=paste0(args[2],"_DMR_Age_fdrs.txt"),sep="\t",row.names=F,quote=F)
