...
dd_CH = {}
spe = "Macaq"
flist = glob.glob("/nv/hp10/hjeong84/data/Projects/WGBS_NHP/CH_methylation/0_preprocessing/CG_context/"+spe+"/CpG_context_*.txt.gz")
wd = '/nv/hp10/hjeong84/data/Projects/WGBS_NHP/CH_methylation/0_preprocessing/CG_context/'+spe+'/'
new_loc = '/nv/hp10/hjeong84/data/Projects/WGBS_NHP/CH_methylation/0_preprocessing/chromosomal_CG_context/'+spe+'/'
for sfile in flist:
    input_file = sfile.split('/')[-1]
    sname = input_file.split('CpG_context_')[-1].split('_processed')[0].split('_bismark')[0]
    new_file = 'CpG_context_'+sname+'.txt.gz'
    fout = open('tmp2/remove_non_chrom_'+sname+'.sh','w')
    fout.write("zcat "+wd+input_file+" | awk '{if ($3 !~ /_/) print $0}' | gzip -c > "+new_loc+new_file)
    fout.close()
    systemstr = 'echo "cd '+wd+' && source activate DSS && '
    systemstr = systemstr + 'sh /nv/hp10/hjeong84/data/Projects/WGBS_NHP/CH_methylation/code/tmp2/remove_non_chrom_'+sname+'.sh'
    systemstr = systemstr+'" | qsub -N '+sname+' -q bioforce-6 -l nodes=1:ppn=1,walltime=20:00:00,mem=16gb'
    print (systemstr)
    os.system(systemstr)
