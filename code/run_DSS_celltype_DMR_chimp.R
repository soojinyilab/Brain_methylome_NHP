...
DMRtest <- callDMR(DMLtest, p.threshold=0.005,minCG=4)
DMRtest_filtered <- DMRtest[(abs(DMRtest$areaStat) / DMRtest$nCG) > abs(pval_max$stat),]
write.table(DMRtest,file=paste0(args[2],"_Celltype_chimp.DMR.fdrs.txt"),sep="\t",row.names=F,quote=F)
write.table(DMRtest_filtered,file=paste0(args[2],"_Celltype_chimp.DMR.fdrs.filtered.txt"),sep="\t",row.names=F,quote=F)
