...
in_dir = "../7_results_celltype_DMR/1_avg_frac_methyl_table/tmp/"
files = glob.glob(in_dir+"HCM_chrchr1_*_Celltype*.meth")
#out_dir = "../7_results_species_DMR/2_fdr_DSS_output/tmp/"
avg_methyl_in_dir = "../7_results_celltype_DMR/1_avg_frac_methyl_table/"
DMR_out_dir = "../7_results_celltype_DMR/2_DMR_output/tmp/"
'''
...
for sfile in files:
    ...
    sfile_index_front2 = "../6_DSS_output_ortho_cytosine/combined_celltype/"+sfile.split('_chrchr')[0].split('/')[-1]+"_chrchr"
    ...
    systemstr = " && cat header_celltype.txt "+' '.join(flist_chrs)+" > "+avg_methyl_in_dir+sfile.split('/')[-1].split('_chrchr')[0]+sfile_index_end
    ...
    run_systemstr = run_systemstr+'" | qsub -N '+sfile.split('/')[-1].split('.')[0]+' -q bioforce-6 -l nodes=1:ppn=1,walltime=5:00:00,mem=32gb'
    print (run_systemstr)
