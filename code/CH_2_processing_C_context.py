...
dd_CH = {}
spe = "Macaq"
flist = glob.glob("/nv/hp10/hjeong84/data/Projects/WGBS_NHP/CH_methylation/0_preprocessing/chromosomal_merged_CH_context/"+spe+"/CH_context_*.txt.gz")
wd = '/nv/hp10/hjeong84/data/Projects/WGBS_NHP/CH_methylation/0_preprocessing/bedGraph/'+spe+'/'

for sfile in flist:
    input_file = sfile
    sname = input_file.split('/')[-1].split('CH_context_')[-1].split('.txt')[0]+'_CH_bedgraph'
    systemstr = 'echo "cd '+wd+' && source activate DSS && '
    systemstr = systemstr + 'bismark2bedGraph --CX --ample_memory -o '+sname+' '+input_file
    systemstr = systemstr+'" | qsub -N '+sname+' -q bioforce-6 -l nodes=1:ppn=1,walltime=70:00:00,mem=50gb'
    print (systemstr)
    os.system(systemstr)
