...
nCPU = 8
for spe in spes:
    split_liftover_in_dir = "../2_lifted_over_cytosine/identify_duplicated_mapping/tmp/tmp_hg19/"
    flist = glob.glob(split_liftover_in_dir+"split_hg19_"+spe+".*")
    out_dir = "../2_lifted_over_cytosine/identify_duplicated_mapping/tmp/tmp_chr/"
    sh_name = "../2_lifted_over_cytosine/identify_duplicated_mapping/tmp/tmp_sh/"+"chromosome_distribute_"+spe+".sh"
    fout = open(sh_name,'w')
    for sfile in flist:
        fout.write('python2 identify_duplicate_split_by_chrom_hg19.py '+sfile+'\n')
    fout.close()
